...
url_postcode_io = "https://api.postcodes.io/postcodes?lon=" + longitude + "&lat=" + latitude;

fetch(url_postcode_io)
.then( response => {
...
function updateStats(area) {
	...
	let last_updated = specimenDates[0]._d;
	...
	last_updated_element.innerHTML = ("Last Updated " + moment(last_updated).format("MMMM Do YYYY"));
}
